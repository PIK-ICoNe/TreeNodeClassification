...
function nodes_and_Leafs(g::Graph, maxiter::Int)
    ...
    for lvl in 1:maxiter
        ...
        Leafs = findall(degree(g) .== 1) # Leafs only have one neighbour
        ...
        append!(tree_nodes, map_to_inital_graph(node_map, Leafs)) # all Leafs are part of a tree
...
function full_node_classification(g::Graph, maxiter::Int, thershold::Int)
    ...
    Leafs_per_lvl, tree_nodes, parents = nodes_and_Leafs(g, maxiter)
    sprouts = []
    ...
    # sprouts are proper Leafs which are adjectent to roots
